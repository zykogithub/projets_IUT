...
def obtenirCA(ca: pd.DataFrame, i: int): return ca.iloc[i].apply(lambda x: x * ECHELLE_POINT ).to_list()

def creerChampGroupe(df: pd.DataFrame): return df.columns[4:-2].to_list()

def filtrerSoiMemeEtGroupesProf(listContatene : list[Groupe], groupe : Groupe) :
    listContatene.pop()
    listContatene.pop(4)
